...
function updatePositions() {
  ...
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin(theThing + (i % 5));
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Try getting screen dimensions to calculate max. fitting pizza quantity.
  var height = window.innerHeight
  || document.documentElement.clientHeight
  || document.body.clientHeight;

  var pizzaQuantity = (height * 8) / 100;

  // Moved shared element to outside for loop
  // to decrease querySelector api calls
